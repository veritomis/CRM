...
class LeadsArgentinaController extends Controller
{
    /**
...
<?php
        ...
        $leads_argentina = Leadsargentina::where('cotizacion', "0")->get();

        $leads_argentina = LeadsArgentina::where('cotizacion', "0")
        ...
        if (session('success') == 1) {
            ...
            foreach ($leads_argentina as $lead) {
                $lead->num_acciones = $lead->accionesargentina()->count();
            }

            // Resto de la lógica del controlador
            return view('leads_argentina.index', [
                'leads_argentina' => LeadsArgentina::orderBy('created_at', 'desc')->get(),             
                'variable' => $variable
            ]);
        } else {
            return view('login');
    ...
    public function create(): View
    {
        ...
        if (session('success') == 1) {
            $variable = session('email');
            // Resto de la lógica del controlador
            return view('leads_argentina.create')->with($variable);
        } else {
            return view('login');
        }
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $input = $request->all();
        $leads_argentina = LeadsArgentina::create($input);
        $ultimoIdIngresado = $leads_argentina->id;

        $Historial = new Historial();
        $Historial->persona_id = $ultimoIdIngresado;
        $Historial->usuario_id = session('ids');
        $Historial->actividad_realizada = "Se agrego un presupuesto (Argentina)";
        ...
        $reporte->descripcion = "\n Leads Argentina: Se agrego un nuevo Leads con el id-> {$ultimoIdIngresado}"; // Descripción de los cambios separados por nueva línea
        ...
        if (session('success') == 1) {
            ...
            return redirect('leads_argentina')->with('leads_argentina', $leads_argentina)->with('variable', $variable);
        } else {
            return view('login');
        }
    }
    ...
    {
        $leads_argentina = leadsArgentina::findOrFail($id);

        // Obtener los apoderados relacionados con la persona jurídica
        $acciones_argentina = accionesArgentina::where('lead', $id)->get();
        ...
        if (session('success') == 1) {
            $variable = session('email');
            // Resto de la lógica del controlador
            return view('leads_argentina.show', [
                'leads_argentina' => $leads_argentina,
                'acciones_argentina' => $acciones_argentina,
                'usuarios' => $usuarios,
                'variable' => $variable
            ]);
        } else {
    ...
    public function edit(string $id): View
    {
        ...
        if (session('success') == 1) {
            // Resto de la lógica del controlador
            return view('leads_argentina.edit', [
                'leads_argentina' => $leads_argentina,
                'variable' => $variable
            ]);
        } else {
    ...
    public function update(Request $request, string $id): RedirectResponse
    {
        ...
        $Historial->usuario_id = session('ids');
        $Historial->actividad_realizada = "Se edito un presupuesto (Argentina)";
        $Historial->fecha_hora = Carbon::now()->format('Y-m-d'); // Establecer la fecha y hora actual
        ...
        $argentina_argentina = leadsArgentina::find($id);
        ...
        if (session('success') == 1) {
            ...
            return redirect('leads_argentina')->with('flash_message', 'leads_argentina Actualizado!', $variable);
        } else {
            return view('login');
        }
    }
    ...
    public function destroy(string $id): RedirectResponse
    {
        ...
        en la seccion Leads Argentina"; // Descripción de los cambios separados por nueva línea
        ...
        return redirect('leads_argentina')->with('flash_message', 'leads_argentina borrado!');
    }
    public function solicitarCotizacion($id)
    {
        // Buscar el lead por el ID
        $lead = leadsArgentina::find($id);
        $nombreC = $lead->company_name;
        $Historial = new Historial();
        $Historial->persona_id = $id;
        $Historial->usuario_id = session('ids');
        $Historial->actividad_realizada = "Se solicito un presupuesto (Argentina)";
        $Historial->fecha_hora = Carbon::now()->format('Y-m-d'); // Establecer la fecha y hora actual
        ...
        if ($lead) {
            ...
            return redirect('leads_argentina')->with('flash_message', 'Cotizacion solicitada!');
        } else {
        }
    }
}
